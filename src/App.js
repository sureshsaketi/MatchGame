...
class App extends Component {
  ...
  onClickImage = thumbnailId => {
    const {imageUrl} = this.state
    const imageDetails = imagesList.filter(
      eachImage => eachImage.imageUrl === imageUrl,
    )
    if (imageDetails[0].id === thumbnailId) {
      const newImage =
  ...
  onResetGame = () => {
    this.setState({isGameOver: false, score: 0, timer: 60})
  }

  render() {
    const {activeTabId, timer, score, imageUrl, isGameOver} = this.state
        ...
        <ul className="images-container">
          {filteredImagesList.map(eachImage => (
            <li
              ...
              onClick={() => this.onClickImage(eachImage.id)}
            >
      ...
      <>
        <div className="app-container">
          ...
          <div className="lower-container">
            {isGameOver ? resultCard() : renderImagesContainer()}
          </div>
        </div>
      </>
    )
